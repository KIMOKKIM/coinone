...
class SignalEngine:
    \"\"\"Generates buy signals based on EMA20 recovery, volume and box filters.\"\"\"
    def __init__(self):
        pass

    def get_buy_signals(self, df: pd.DataFrame, box: Dict[str, Any]) -> List[Dict[str, Any]]:
        signals = []
        df = df.copy()
        df["ema20"] = Indicators.ema(df["close"], cfg.EMA_PERIOD)
        df["volume_ma"] = Indicators.volume_ma(df["volume"], cfg.VOLUME_MA_PERIOD)
        recent_high = box.get("highs") if box else None
        for i in range(len(df)):
            row = df.iloc[i]
            sig = {"index": i, "trend_ok": False, "final_buy_signal": False}
            price = float(row["close"])
            ema = float(row["ema20"]) if not pd.isna(row["ema20"]) else None
            vol = float(row["volume"])
            vol_ma = float(row["volume_ma"]) if not pd.isna(row["volume_ma"]) else None
            # trend filter: price above EMA20
            if ema is not None and price > ema:
                sig["trend_ok"] = True
            # recovery check: previous close below EMA and current above (requires i>=1)
            if i >= 1 and ema is not None:
                prev = df.iloc[i-1]
                prev_close = float(prev["close"])
                prev_ema = float(df["ema20"].iloc[i-1]) if not pd.isna(df["ema20"].iloc[i-1]) else None
                recovered = prev_ema is not None and prev_close < prev_ema and price > ema
            else:
                recovered = False
            # volume spike
            vol_ok = False
            ...
            # don't buy near recent high (avoid chase)
            near_high = False
            if recent_high is not None and not pd.isna(recent_high.iloc[i]):
                high_val = float(recent_high.iloc[i])
                if high_val > 0 and price > high_val * 0.98:
                    near_high = True
            # final buy logic: trend_ok and (recovered or (price crossed ema and vol spike)) and not near high
            if sig["trend_ok"] and not near_high and (recovered or (price > ema and vol_ok)):
                sig["final_buy_signal"] = True
            signals.append(sig)
        return signals
